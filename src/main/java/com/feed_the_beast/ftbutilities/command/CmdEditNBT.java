...
public class CmdEditNBT extends CmdTreeBase {
	...
	private static void addInfo(NBTTagList list, IChatComponent key, IChatComponent value) {
		list.appendTag(
				new NBTTagString(
						IChatComponent.Serializer.func_150696_a(StringUtils.color(key, EnumChatFormatting.BLUE)
								.appendText(": ").appendSibling(StringUtils.color(value, EnumChatFormatting.GOLD)))));
	}
	...
	private static class CmdBlock extends CmdNBT {
		...
		public NBTTagCompound editNBT(EntityPlayerMP player, NBTTagCompound info, String[] args)
			...
			if (tile != null) {
				...
				addInfo(list, new ChatComponentText("ID"),
						new ChatComponentText(key == null ? "null" : key.toString()));
				addInfo(list, new ChatComponentText("Block"),
						new ChatComponentText(tile.getBlockType().getLocalizedName()));
				addInfo(list, new ChatComponentText("Block Class"),
						new ChatComponentText(tile.getBlockType().getClass().getName()));
				addInfo(list, new ChatComponentText("Position"),
				...
				addInfo(list, new ChatComponentText("Mod"),
						new ChatComponentText(mod == null ? "null" : mod.getName()));
				addInfo(list, new ChatComponentText("Ticking"),
	...
	private static class CmdEntity extends CmdNBT {
		...
		public NBTTagCompound editNBT(EntityPlayerMP player, NBTTagCompound info, String[] args)
			...
			if (entity != null) {
				...
				addInfo(list, new ChatComponentText("ID"),
						new ChatComponentText(key == null ? "null" : key));
				...
				addInfo(list, new ChatComponentText("Mod"), new ChatComponentText(mod == null ? "null" : mod.getName()));
				info.setTag("text", list);
	...
	private static class CmdPlayer extends CmdNBT {
		...
		public NBTTagCompound editNBT(EntityPlayerMP player, NBTTagCompound info, String[] args)
			...
			info.setTag("text", list);
			info.setString("title", IChatComponent.Serializer.func_150696_a(
					new ChatComponentText(player.getDisplayName())));
			return nbt;
		}
	}
